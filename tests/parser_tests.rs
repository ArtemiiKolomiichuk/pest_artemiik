...
mod parser{
    ...
    #[test]
    fn file_iterates() -> anyhow::Result<()>{
        let mut iter = Grammar::parse(Rule::file, "-44.43,-15\n-44.43,-15\n")?;
        let data = iter.next().ok_or_else( || anyhow::anyhow!("no 'file'"))?;
        assert_eq!(data.as_str(), "-44.43,-15\n-44.43,-15\n");
        Ok(())
    }
    ...
    #[test]
    fn wrong_file_is_err() -> anyhow::Result<()>{
        let res = Grammar::parse(Rule::file, "-44.43 15\n");
        assert!(res.is_err());
        Ok(())
    }
}
